...
% Berkshire Hathaway stock price in the last five years

% Data from Yahoo Finance

T=readtable('BRK-B.csv');  % Table columns: Date , Open , High , Low , Close , AdjClose , Volume
...
figure;
plot(P),title('Berkshire Hathaway stock price in the period 27/06/2017-24/06/2022');

figure;
autocorr(P,120),title('ACF of Berkshire Hathaway P');
...
figure;
plot(y),title('Berkshire Hathaway stock returns as diff(log(P))');

figure;
subplot(3,1,1),autocorr(y,80),title('ACF Berkshire Hathaway stock returns');
subplot(3,1,2),autocorr(abs(y),80),title('ACF absolute value of Berkshire Hathaway stock returns');
subplot(3,1,3),autocorr(y.^2,80),title('ACF squared Berkshire Hathaway stock returns');
...
s=skewness(y);   % s = -0.366

k=kurtosis(y);   % k = 14.45 >> 3
...
% Significant autocorrelation of innovation errors 

% 2.2 Beta-t-Garch(1,1)

% par_order=phi, nu, mu, delta, kappa
par0 = [0.1  4  0.001   0.1  0.1];
...
% Estimated degrees of freedom hat_nu=5.1052(<<300) indicates that the Student-T 
